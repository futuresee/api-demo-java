...
public class HttpUtil {
    ...
    public static String getContractSignature(
            String url,
        ...
        try {
            ...
            if (queryParams != null && !queryParams.isEmpty()) {
                ...
                sb.append("#");
                for (String k : keyArray) {
                    sb.append(k).append("=").append(queryParams.get(k).toString().trim()).append("&");
                }
                sb.deleteCharAt(sb.length() - 1);
            }
            if (requestBody != null && !requestBody.isEmpty()) {
                sb.append("#").append(JSONObject.toJSONString(requestBody));
//                Set<String> keySet = requestBody.keySet();
//                String[] keyArray = keySet.toArray(new String[keySet.size()]);
//                Arrays.sort(keyArray);
//                sb.append("#");
//                for (String k : keyArray) {
//                    sb.append(k).append("=").append(requestBody.get(k).toString().trim()).append("&");
//                }
//                sb.deleteCharAt(sb.length() - 1);
            }
            System.out.println(sb.toString());
            System.out.println(HmacUtils.hmacSha256Hex(secretKey, sb.toString()));
            return HmacUtils.hmacSha256Hex(secretKey, sb.toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
